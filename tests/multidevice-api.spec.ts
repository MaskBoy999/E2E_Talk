...
test.describe('Multi-Device Security API', () => {
    ...
    test('POST /api/devices/escrow stores per-device escrowed key', async ({ page }) => {
        const ts = Date.now();
        const username = 'escrow_' + ts;
        const { token } = await registerUser(page, username);

        const deviceId = 'escrow-device-' + ts;
        const keyPair = await page.evaluate(() => {
            const kp = E2ECrypto.x25519GenerateKeyPair();
            return E2ECrypto.arrayBufferToBase64(kp.publicKey);
        });
        ...
        await page.request.post(`https://localhost:3443/api/devices`, {
            headers: { Authorization: `Bearer ${token}` },
            data: { device_id: deviceId, device_name: 'Escrow Test', identity_key: keyPair },
        });

        // Upload per-device escrowed key (server stores blobs, no validation)
        const escrowRes = await page.request.post(`https://localhost:3443/api/devices/escrow`, {
            headers: { Authorization: `Bearer ${token}` },
            data: { encrypted_private_key: 'dGVzdA==', salt: 'dGVzdA==', nonce: 'dGVzdA==', device_id: deviceId },
        });
        expect(escrowRes.ok()).toBeTruthy();
    });

    test('GET /api/devices/escrow retrieves per-device escrowed key', async ({ page }) => {
        const ts = Date.now();
        const username = 'escrow_get_' + ts;
        const { token } = await registerUser(page, username);

        const deviceId = 'escrow-get-device-' + ts;
        const keyPair = await page.evaluate(() => {
            const kp = E2ECrypto.x25519GenerateKeyPair();
            return E2ECrypto.arrayBufferToBase64(kp.publicKey);
        });
        ...
        await page.request.post(`https://localhost:3443/api/devices`, {
            headers: { Authorization: `Bearer ${token}` },
            data: { device_id: deviceId, device_name: 'Escrow Get', identity_key: keyPair },
        });

        // Upload escrow with unique data
        const testB64 = Buffer.from('test_private_key_' + ts).toString('base64');
        const saltB64 = Buffer.from('test_salt_' + ts).toString('base64');
        const nonceB64 = Buffer.from('test_nonce_' + ts).toString('base64');

        const uploadRes = await page.request.post(`https://localhost:3443/api/devices/escrow`, {
            headers: { Authorization: `Bearer ${token}` },
            data: {
                encrypted_private_key: testB64,
                salt: saltB64,
                nonce: nonceB64,
                device_id: deviceId,
            },
        ...
        // Retrieve escrow
        const getRes = await page.request.get(`https://localhost:3443/api/devices/escrow`, {
            headers: { Authorization: `Bearer ${token}`, 'x-device-id': deviceId },
        });
        expect(getRes.ok()).toBeTruthy();
        const retrieved = await getRes.json();
        expect(retrieved.encrypted_private_key).toBe(testB64);
        expect(retrieved.salt).toBe(saltB64);
        expect(retrieved.nonce).toBe(nonceB64);
        expect(retrieved.device_id).toBe(deviceId);
    });

    test('device removal cascades to per-device escrow', async ({ page }) => {
        const ts = Date.now();
        const username = 'dev_cascade_' + ts;
        const { token } = await registerUser(page, username);

        const deviceId = 'cascade-device-' + ts;
        const keyPair = await page.evaluate(() => {
            const kp = E2ECrypto.x25519GenerateKeyPair();
            return E2ECrypto.arrayBufferToBase64(kp.publicKey);
        });

        // Register device + upload escrow
        await page.request.post(`${BASE}/api/devices`, {
            headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
            data: { device_id: deviceId, device_name: 'Cascade', identity_key: keyPair },
        });
        await page.request.post(`${BASE}/api/devices/escrow`, {
            headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        // Delete device
        await page.request.delete(`${BASE}/api/devices/${deviceId}`, {
            headers: { Authorization: `Bearer ${token}` },
        });

        // Verify escrow is gone
        const getRes = await page.request.get(`${BASE}/api/devices/escrow`, {
            headers: {
    ...
    test('GET /api/devices/escrow without device_id falls back to user-level escrow', async ({ page }) => {
        const ts = Date.now();
        const username = 'esc_fb_' + ts;
        const { token } = await registerUser(page, username);

        // Upload user-level escrow (different endpoint)
        const escrowRes = await page.request.post(`https://localhost:3443/api/identity/escrow`, {
            headers: { Authorization: `Bearer ${token}` },
            data: {
                encrypted_private_key: 'dGVzdA==',
                salt: 'dGVzdA==',
        ...
        // GET /api/devices/escrow without x-device-id should fallback to user-level escrow
        const getRes = await page.request.get(`https://localhost:3443/api/devices/escrow`, {
            headers: { Authorization: `Bearer ${token}` },
        });
        expect(getRes.ok()).toBeTruthy();
