...
| 07-30 | XSS protection — escapeHtml made globally available | `escapeHtml`/`escapeAttr` were moved from `chat.js` to `auth.js`/`admin.js` during security upgrade, but `index.html` doesn't load `auth.js`, causing `ReferenceError: escapeHtml is not defined` → messages silently failed to render. Fixed by adding both as `window.*` globals in `secure-storage.js` (loaded by ALL pages). Also fixed server-side `message_signature` column reference: migration 044 dropped the column but Rust INSERT/UPDATE/SELECT still referenced it, causing "table messages has no column named message_signature" errors. Removed all `message_signature` references from `db.rs` and `ws.rs`. Full XSS audit completed: 5-layer protection (escapeHtml, escapeAttr, renderEmojiText, highlightMentionsInHtml, linkifyText after escapeHtml), no reflected XSS, no eval/document.write, no javascript: protocol injection, all 80+ innerHTML assignments verified. |
| 07-30 | Null-overwrite audit: found and fixed 3 classes of bugs in `profile_updated` WS handler, `fetchAndCacheUserProfile`, `fetchServerConversationProfile`, `fetchDmConversationProfile`. All had `if (value)` or `value || fallback` patterns that treated `null` as falsy — preventing PFP/banner removal from propagating to `myProfile` and `userDisplayNameCache`. Changed all to `!== undefined` so explicit `null` (sent by server after removal) propagates correctly.
| 07-30 | Added 'Remove PFP' / 'Remove Banner' buttons in profile editor. Uses sentinel flags (`_removePfpFlag`/`_removeBannerFlag`) to distinguish "not changed" from "remove". When flag is set: profileData stores `null` for file_id/key, HTTP body sends `remove_picture: true`/`remove_banner: true`, server nulls the DB columns. Added `remove_banner` field + handler to `UpdateProfileRequest` in handlers.rs. Flags reset on new upload and when editor opens.
| 07-30 | Fixed profile PFP/banner overwrite bug: `saveProfile()` was storing `null` for un-changed fields (`profile_picture_file_id`/`profile_picture_file_key`/`profile_banner_file_id`/`profile_banner_file_key`) because the editor globals reset to `null` on modal open. Fixed by falling back to current `myProfile` values. E.g. `profilePfpFileId || (myProfile && myProfile.profile_picture_file_id) || null`. Now updating only banner preserves PFP and vice versa.
| 07-30 | Added floating 'Jump to latest' button — fades in when scrolled >200px from bottom, smooth-scrolls to latest messages on click. Works for both channels and DMs. ~45 lines in chat.js + ~30 lines CSS in style.css.
| 07-30 | Column-index audit: all 80+ `query_map`/`query_row` closures in `db.rs` verified — **zero off-by-one bugs**. Fixed 2 cosmetic inconsistencies: `list_dm_messages`/`list_dm_messages_before` had `file_id: row.get(22).ok().flatten()` (out-of-bounds, always returned None via error swallow) → changed to `file_id: None` to match channel message equivalents.
| 07-30 | Fix: secure-storage encryption timing — `e2e_*` keys stored before `_secReKey()` broke registration | The secure-storage `Storage.prototype` interceptor encrypts all `e2e_*` keys. During registration/login, these keys were stored BEFORE `_secReKey()`, so they got encrypted with the random fallback key. After `_secReKey()` switched to the password-derived key, `index.html` couldn't decrypt them — identity keys, HMAC key, auth key, and friend code all returned null. Fixed by moving ALL `e2e_*` `setItem` calls (identity keys, auth_key, hmac_key, friend_code) to AFTER `_secReKey()` in both login and registration flows. Also added `e2e_friend_code` to `BOOTSTRAP_KEYS` (friend codes are meant to be shared, no need to encrypt). Auth test registration pass rate went from 0→4/5 (remaining failure: escrow routes are dead code — handler exists but route NOT registered in main.rs). |
| 07-31 | Fix: messages disappearing after page refresh — `row.get()` column index mismatch | Migration 044 dropped `message_signature` from all SQL SELECTs but Rust `row.get()` indices weren't updated. All indices from `encrypted_profile_key` onward were off by 1 (e.g. `banner_key_nonce: row.get(14)?` read INTEGER column `key_version` as `Option<String>`). `filter_map(|r| r.ok())` silently swallowed errors → REST API returned `[]`. Fixed indices in all 10 affected message-reading functions. |
| 07-31 | Modal dual-mode UI: key recovery vs friend code | `showFriendCodePasswordModal()` now dynamically switches title, description, button text, and hides "Generate New" based on whether identity keys are missing. Uses `identityMissing` flag re-evaluated on every modal open. HTML defaults made generic. |
| 07-31 | Key blob re-save after modal recovery | `_recoverFromBlob()` calls `saveKeyBlobToServer()` after restoring keys to persist the merged state (recovered identity keys + newer session keys). Re-stores `e2e_encrypted_password` after `_secReKey()` to avoid secure-storage key mismatch. |
| 07-31 | Fix: DM profile not refreshing in sidebar and chat header | Added `refreshDmProfileDisplay(userId)` helper that updates the DM chat header (display name, profile pic, presence dot) with proper escapeHtml(). Wired into `fetchServerConversationProfile`, `fetchDmConversationProfile`, and the WS `profile_key_sync` handler. Sidebar is updated separately via existing `refreshDmSidebarItem()` (WS handler) and `renderDmSidebar()` (loadDmConversations). XSS protection verified intact — `escapeHtml`/`escapeAttr` used throughout. |
| 07-31 | Dev script: `restart-server.sh` / `restart-server.bat` | One-command dev loop: kills the old `e2e-chat` process (by image name + LISTENING sockets on 3000/3443 only — never client PIDs), rebuilds with `cargo build` (prints BUILD FAILED and exits 1 on error), then restarts. Both launch the binary from `server/` so the CWD-relative `e2e_chat.db` resolves to `server/e2e_chat.db`, matching `start-server.bat` and Playwright's webServer (earlier version ran from project root, which silently opened a different empty root-level DB — fixed). Bash uses `if ! ( cd server && cargo build )` so the error prints under `set -e`; batch pipes through `findstr LISTENING`. Validated end-to-end. |
| 07-31 | Fix: `file_key_nonce: None` in edit functions — data loss on edited messages with file attachments | `edit_encrypted_message` and `edit_dm_message` both SELECT `m.file_key_nonce` (column index 16) but ignored it in the Rust struct constructor, setting `file_key_nonce: None`. On the client side this meant edited messages with file attachments would lose their file decryption key after edit. Fixed both to `file_key_nonce: row.get(16)?`. |
| 07-31 | Fix: DM REST serialized `profile_snapshot_nonce`/`encrypted_file_key`/`file_key_nonce` as raw JSON number arrays — broke snapshot-based name colors/glow on DM messages loaded via REST (initial load + infinite scroll) | `list_dm_messages` in handlers.rs emitted `Option<Vec<u8>>` fields raw (JSON arrays) while the channel handler (`list_messages`) and the WS broadcast base64-encoded them. `appendDmMessage` fed the array nonce into `decryptDm` → snapshot decrypt failed → sender display name color + glow silently missing whenever the per-message server `conversation_profile` didn't supply it. Fixed all 4 fields to `.as_ref().map(|v| base64 encode)` matching the channel handler + WS broadcast. Added `normalizeB64Field()` in chat.js (hoisted, idempotent, null-safe) wired into the snapshot-decrypt blocks of `appendMessage`/`appendDmMessage` so stale/cached array nonces self-heal. New test `tests/verify-scroll-profile.spec.ts` covers BOTH DM and channel: colored sender + 60 msgs, asserts the raw REST serialization guard (nonce is base64 string, not array, with presence assertions so the guard can't go dormant) AND colored/glowing display names on all 60 prepended messages after scroll-to-top. |
| 07-31 | Fix: infinite-scroll sender not identified/shown | Three compounding client bugs in `chat.js`. (1) **Grouping hid the sender header**: `loadMoreMessages`/`loadMoreDmMessages` saved/restored `lastMessageInfo`/`lastDmMessageInfo` but only reset it *after* the prepend loop, so the first prepended message was compared against the *newest* initial-load message (non-adjacent). Same-sender + <2min → wrongly got `.grouped`, and `.grouped .header { display:none }` hid the sender name. Fixed by resetting grouping state *before* the prepend loop (within-batch grouping still works since the state updates per iteration). (2) **No profile prefetch in pagination**: initial loads call `fetchServerConversationProfile`/`fetchDmConversationProfile` for uncached senders but the scroll paths didn't, so senders only present in older batches never got display name/colors/PFP. Added matching prefetch loops. (3) **DM cache-key bug**: `appendDmMessage` looked up `userDisplayNameCache[msg.sender_id]` (HMAC'd id) but the cache is keyed by raw UUID — DM sender styling/names never applied from cache. Switched to `senderIdForCache` with dual-key storage for PFP/banner keys, matching the channel path. |
| 07-31 | Fix: own messages missing edit/delete buttons (isOwn always false) | `isOwn` in `appendMessage`/`appendDmMessage` was computed purely as `msg.sender_id === mySenderId` — an HMAC-SHA256 comparison using `e2e_hmac_key` from localStorage. If that key is missing or stale (e.g., session predating the key, or secure-storage returning null), `mySenderId` falls back to the raw UUID and never matches the HMAC'd `sender_id`, so every own message rendered without edit/delete buttons. Fixed by preferring the raw-UUID comparison `msg.sender_user_id === myUserId` (the server sends the raw UUID as `sender_user_id` on channel REST, DM REST, and WS — same pattern already used in the WS `dm_new` handler), with the HMAC comparison kept as fallback for messages lacking `sender_user_id`. |
| 07-31 | Verified both infinite-scroll fixes end-to-end | New test `tests/verify-scroll-sender.spec.ts`: registers a user, creates a server, sends 60 messages via WS (mimicking the real UI send flow incl. `encrypted_sender_username`), opens the channel, and asserts the initial 50-message batch AND the batch loaded after scrolling to top all render non-empty sender names and own-message edit/delete buttons. Result: 50/50 initial, then 60/60 after pagination — all names non-empty, all edit/delete buttons present. Passed (21.6s). |
| 08-01 | Verified snapshot-only repaint path + added snapshot-only tests | Confirmed repaint timing: `fetchServerConversationProfile`/`fetchDmConversationProfile` are fired async AFTER prepending in `loadMoreMessages`/`loadMoreDmMessages`, and both end with `updateExistingMessageStyles(userId)` (chat.js:428-430, 487-489), so prepended messages repaint when the fetch resolves. Diagnosed why the earlier "snapshot-only" test saw `conversation_profile present: true`: the server broadcasts `friend_request_accepted` to BOTH users (handlers.rs:4441) and the client's WS handler calls `uploadCurrentProfileToConversations()` (chat.js:5681) — auto-uploading conversation profiles that masked the DM REST serialization bug in every prior test. Added to `tests/verify-scroll-profile.spec.ts`: `blockConversationProfileUploads()` (page.route aborts PUT `**/api/profile/conversation`) + `patchProfileOnly()` (PATCH /api/profile only), and two new snapshot-only tests (DM + channel) that block conversation-profile uploads, assert the raw REST `conversation_profile` is `null`, and verify colored/glowing display names on all 60 prepended messages come purely from the per-message `encrypted_profile_snapshot`. Also found + fixed a real test bug: `buildSnapshot` was declared `async` but called without `await`, so `JSON.stringify(Promise)` produced `"{}"` empty snapshots — made it a plain sync function (with a comment warning) so the foot-gun can't recur. All 6 tests in the two scroll specs pass (60/60 correctName/colored/withGlow after scroll). |
| 08-01 | Fix: sticker/emoji/GIF upload crash — "table user_stickers has no column named sticker_name" | Migration 045 dropped the plaintext `sticker_name` column (confirmed: live DB has no such column), but Rust still referenced it in 3 places: `add_user_sticker` INSERT (10 cols → 9), `list_user_stickers` SELECT (9-tuple → 8), and `list_all_user_stickers_admin` SELECT (11-tuple → 10). Fixed all three in `db.rs` (removed column/param/tuple/index positions), plus `handlers.rs`: removed `sticker_name` from `AddUserStickerRequest`, the `add_user_sticker` call, `list_user_stickers` destructuring, and `admin_list_user_stickers` (destructuring + JSON output). Client still sends `sticker_name` in the upload payload — serde ignores unknown fields, harmless. Sticker names remain fully encrypted (`encrypted_sticker_name` + nonce, migration 041) and the client decrypts into `s.sticker_name` on load, so the emoji/sticker caches are unaffected. |
| 08-01 | Fix: channel/DM load lands a few messages short of latest — always jump to newest on load/reload | `appendMessage`/`appendDmMessage` scroll to bottom per-message, but async image/sticker loads grow the list AFTER the scroll, so the view ended up 2-3 messages from the end. Added a pinned-to-bottom state to `chat.js`: `_pinnedToBottom` flag + `scrollMessageListToBottom()` (pins + scrolls + hides the jump button) + `setupImageLoadRepin()` (capture-phase `load` listener that re-scrolls to bottom when pinned and an IMG finishes loading, attached once via `_imageRepinAttached`). Both scroll listeners (channel + DM) unpin when the user scrolls away (>60px from bottom). Called at the end of `loadMessages` (only when `!aroundMessageId`) and unconditionally at the end of `loadDmMessages`. Also fixed: (1) Ctrl+Shift+M media-preview toggle was calling `loadMessages(currentChannelId, currentServerId)` — passing the server ID as `aroundMessageId` and hitting `/around/{serverId}` — now `loadMessages(currentChannelId)`; (2) message-jump flows (selectChannel/selectDmChannel with `messageId`) now set `_pinnedToBottom = false` before `scrollIntoView` so image loads don't yank the view back to the bottom (regression the code reviewer caught). |
| 08-01 | Test fixes in `tests/stickers-gifs.spec.ts` + sticker suite 10/10 | (1) DIAGNOSTIC emoji test read the removed plaintext `sticker_name` from the raw API — now decrypts `encrypted_sticker_name` via `page.evaluate` (single object arg + `window.E2ECrypto`, var-scoped) and asserts `image/emoji`. (2) DM test was calling dead routes `/api/friend-requests/*` (route is `/api/friends/request`, `/api/friends/requests/incoming`, `/api/friends/requests/accept`) — updated to the registered endpoints. (3) DM test created user2 in the SAME browser context, so shared localStorage overwrote `e2e_friend_code` → friend request failed with "You can't add yourself as a friend" while the test PASSED VACUOUSLY (assertions nested inside `if` guards) — now creates user2 in a separate `browser.newContext()`, asserts `frRes.ok()` loudly, and closes ctx2. Full suite: **10/10 passed**. |
| 08-01 | Security audit: no plaintext copies, no key-with-ciphertext workaround (code audit, no tests run) | Verified the E2EE promise end-to-end after all fixes. **Message content (channel + DM)**: WS `message_send`/`dm_send` handlers accept ONLY `encrypted_content` + `nonce` (+`message_nonce`); server stores ciphertext only, never a key. **DM keys**: never stored server-side — `dm_keys` table was DROPPED in migration 022; the key is derived client-side via X25519 ECDH + HKDF per channel, so the host has no DM key at all. **Server/channel keys**: `server_keys` stores `encrypted_key` envelope-encrypted per member with that member's public key + `sender_public_key` + `nonce` — host cannot unwrap without a member's private key. **File uploads**: chunks are client-encrypted (XChaCha20-Poly1305); the file key travels ONLY inside the channel/DM-encrypted message payload (as `encrypted_file_key`), never in the clear. **Profiles**: display name/colors/description/nickname live inside `encrypted_profile_data` (AES-GCM with a dedicated profile_data_key); the PFP/banner file keys are identity-key-wrapped (`encrypted_pic_key`/`encrypted_banner_key`, migrations 036/042/043 dropped all plaintext columns — confirmed 0 rows in legacy `profile_picture_file_key`/`profile_banner_file_key`/`invite_code`). **Escrow/blob**: identity private key + key blob are password-wrapped (Argon2id/HKDF); `user_key_escrow`/`user_key_blobs` store ciphertext + salt + nonce only. **Offline notifications**: `pending_notifications` payloads are envelope-encrypted to the recipient's public key before INSERT. **Admin endpoints**: expose only ciphertext + public keys + hashes (`encrypted_content`, `encrypted_key`, `encrypted_private_key`, `encrypted_blob`) — nothing decryptable. Also confirmed: server names/channels encrypted (`encrypted_name`), sticker names encrypted (`encrypted_sticker_name`), sender usernames encrypted (`encrypted_sender_username`), file MIME types encrypted (`encrypted_mime_type`) with only legacy empty-string fallbacks, file_ids stored as SHA-256 hashes. **No plaintext copy of any user content is stored; no decryption key is ever transmitted alongside ciphertext.** |
| 08-01 | Fix: notification sound upload crash — "table notification_sounds has no column named file_name" | Same bug class as the sticker crash: migration 045 dropped the plaintext `file_name` column (confirmed: live DB `notification_sounds` = user_id, encrypted_sound, nonce, sender_public_key, created_at, updated_at, encrypted_file_name, file_name_nonce — no `file_name`), but `save_notification_sound` INSERT + ON CONFLICT SET clauses + params and `get_notification_sound` SELECTs still referenced it. Fixed in `db.rs`: removed `file_name` from both INSERT branches, both ON CONFLICT SET clauses, and params; both SELECTs now return `''` at tuple position 3 (preserving the `(Vec<u8>,Vec<u8>,Vec<u8>,String,Option<Vec<u8>>,Option<Vec<u8>>)` arity so the handler destructuring is untouched). `handlers.rs`: `upload_notification_sound` call site updated to drop `&req.file_name`; `UploadNotificationSoundRequest.file_name` changed `String` → `Option<String>` (the client's `'encrypted'` placeholder is now tolerated-but-ignored; the real display name lives in `encrypted_file_name`/`file_name_nonce`, migration 039, which the client already decrypts). Rebuilt + restarted (HTTP 200). Two reviewer passes: tuple arity ✓, serde `Option` ✓, envelope encryption (`encrypted_sound` + `nonce` + `sender_public_key`) untouched ✓, exactly one caller ✓. |
| 08-01 | Final plaintext-fallback sweep + fix: files.mime_type no longer stored in plaintext | Audited every column in `files`, `user_stickers`, `servers`, `users`, `notification_sounds` against the live DB (PRAGMA + row counts) and every Rust INSERT/UPDATE path. **Live DB is already clean**: 0 non-empty rows in all legacy plaintext columns (`files.mime_type`, `user_stickers.mime_type`, `servers.invite_code`, `users.profile_picture_file_key`/`profile_banner_file_key`); `notification_sounds.file_name` gone (fixed above). **Two write-paths were still populating plaintext**: (1) `create_file_record` INSERTed the client-supplied plaintext `mime_type` on every file upload even though `encrypted_mime_type`+`mime_nonce` (migration 038) exist — FIXED: the encrypted-column branch now writes `''` into `mime_type` and stores only the ciphertext. Safe because `download_file` serves `application/octet-stream` regardless and the client receives file mime inside the E2E-encrypted message payload (never from this table). Sticker JOINs fall back to `user_stickers.mime_type` which `add_user_sticker` still populates. Placeholder numbering `?1..?6` verified after the reviewer caught an initial `?5..?7` off-by-skip that would have thrown `InvalidParameterCount`. (2) `add_user_sticker` still writes plaintext `mime_type` — **left as-is deliberately**: the client needs it to categorize emoji vs sticker (`mime_type === 'image/emoji'`) and its `encrypted_mime_type`/`mime_nonce` columns exist but are never populated (dead columns). Flagged for a follow-up if MIME-level metadata hiding is desired. `servers.invite_code` is a legacy nullable column only touched by migration backfill (never written by `create_server`, which writes only `invite_code_hash`) — inert. Rebuilt + restarted (HTTP 200). Reviewer-verified: param alignment, legacy branches intact, no client impact, sticker JOIN safe. |
...
| 07-31 | Fix: `row.get()` indices not updated after `message_signature` column drop | Migration 044 removed `message_signature` from all SELECT queries, shifting every column left by 1. But the Rust `row.get()` calls still used the old indices (e.g. `row.get(11)` for `encrypted_profile_key` which was now at column 10). This caused type-mismatch errors (e.g. reading INTEGER column `key_version` as `Option<String>`) that were silently swallowed by `filter_map(|r| r.ok())` → the REST API returned `[]` for all message queries. Fixed all 10 affected functions (`list_messages`, `list_messages_before`, `list_messages_around`, `list_dm_messages`, `list_dm_messages_before`, `edit_encrypted_message`, `edit_dm_message`, admin `list_all_messages_admin`, and DM around queries). Server channel + DM persistence tests now pass. |
| 07-31 | Modal dual-mode UI: key recovery + friend code | `showFriendCodePasswordModal()` now dynamically updates title ("🔑 Key Recovery" vs "🔒 Friend Code Access"), description, recover button text, and hides the "Generate New" button when identity keys are missing. The `identityMissing` flag is re-evaluated on every open so the UI correctly adapts. HTML defaults changed to generic "Account Verification". |
| 07-31 | Key blob re-save after recovery | `_recoverFromBlob()` now calls `saveKeyBlobToServer()` after successful recovery, ensuring the server blob reflects the merged state of restored identity keys plus any newer session keys. Also re-stores `e2e_encrypted_password` **after** `_secReKey()` so the secure-storage interceptor encrypts it with the password-derived key (not the pre-rekey fallback key), preventing a silent decryption failure in `saveKeyBlobToServer()`. |
| 07-31 | Verified DM infinite-scroll variant end-to-end | Extended `tests/verify-scroll-sender.spec.ts` with a DM variant: registers 2 users, becomes friends, sends 60 DM messages via WS using a helper that mimics the real UI flow (content via `encryptDm`, `encrypted_sender_username` encrypted with the DM shared key via `getDmKey`+`encryptSenderUsername`, exactly what ws.rs stores and `appendDmMessage` decrypts). Opens the DM, asserts initial 50-batch has names+edit/delete, scrolls to top, asserts all 60 rendered messages have non-empty sender names and own-message edit/delete buttons. **Both channel + DM variants pass (50.5s)**. |
| 07-31 | Self-healing HMAC key for `isOwn` fallback | `appendMessage`/`appendDmMessage` now call `ensureHmacKey()` when `e2e_hmac_key` is missing from localStorage, so the HMAC'd `sender_id` isOwn fallback can never silently fail on stale sessions (previously `mySenderId` fell back to the raw UUID and own messages lost edit/delete). `ensureHmacKey` gained a single-flight guard (`_hmacKeyFetchPromise`, shared promise + finally reset) so a render burst doesn't hammer `/api/hmac-key`, and it `console.warn`s if the fetch fails so the residual failure is observable. `appendDmMessage` became async; its one previously-unawaited call site (prepend loop in `loadMoreDmMessages`) now awaits it, preserving grouping order and scroll math. Syntax check passes; reviewer approved. |
| 07-31 | Fixed stale `messaging.spec.ts` server-creation helper | All 4 tests in `tests/messaging.spec.ts` were failing at `createServerAndKey` with `Unexpected token 'F', "Failed to "... is not valid JSON`. Root cause: the helper still posted the legacy payload `{ name, invite_code_hash }` but the modern `CreateServerRequest` (handlers.rs) requires `invite_code` (and treats encrypted-name fields as optional, ignoring unknown keys). Fixed to post `{ invite_code }` only (same proven pattern as verify-scroll-sender.spec.ts) and removed the now-unused `sha256Hex`/`createHash`. **4/4 pass now.** Note: bulk Playwright runs trip the server's IP rate limiters (LOGIN_IP 10/5min, FRIEND_REQUEST_IP 10/10min, HMAC_KEY 6/60s) — run suites sequentially with `--workers=1` against a freshly-restarted server for clean results. |
| 07-31 | Fix: PFP/banner clobbering when only one is updated | `saveProfile()` fell back to the stale `myProfile` global for un-changed PFP/banner fields, and the post-save refresh (`data2`) never had `decryptOwnProfileFileKeys()` applied. Fixed: fallback now reads `profileOriginalData.data` (already key-decrypted when the modal opened), and the post-save response is run through `decryptOwnProfileFileKeys(data2)` so the un-edited image's file key survives the save. |
| 07-31 | Fix: infinite-scroll prepend reversed message order + broken pagination | (1) `loadMoreMessages`/`loadMoreDmMessages` prepended the server's ASC batch one-by-one at the top, which reversed each batch (newest ended up above oldest). Fixed with reverse iteration so the list stays chronological. (2) Pagination was timestamp-only, so same-second messages were skipped/duplicated, and WS RFC3339 timestamps differed from DB space-separated `CURRENT_TIMESTAMP` values. Fixed: `save_message`/`save_dm_message` now write a fixed-width RFC3339 timestamp (`%Y-%m-%dT%H:%M:%S%.6fZ`) instead of the table default; `list_messages`/`list_dm_messages` + `list_*_before` use a compound `(timestamp, id)` cursor (`before_id` param passed through handlers); all four list queries tiebreak with `id ASC/DESC`; `list_messages_around` got the same tiebreak. (3) Added an idempotent data migration in `run_migrations` that converts legacy space-separated timestamps (`YYYY-MM-DD HH:MM:SS`, length 19) to RFC3339 so ordering is consistent across the format boundary. (4) DM `loadMoreDmMessages` now checks for the identity key BEFORE advancing the pagination cursor so a missing key can't permanently skip a fetched batch. Server + DM message persistence tests pass. |
| 07-31 | Migration 048: dropped 26 dead legacy columns | App is unreleased, so no back-compat. New `migrations/048_drop_legacy_message_and_key_columns.sql` drops: `messages`/`dm_messages` (7 each: `message_nonce`, `encrypted_profile_key`/`profile_key_nonce`, `encrypted_banner_key`/`banner_key_nonce`, `encrypted_file_key`/`file_key_nonce` — client always sent null; profile/file data now rides in `encrypted_profile_snapshot` + message-payload keys), `users` (8: `encrypted_profile_key`, `profile_eph_pub`, `profile_nonce`, `encrypted_private_key`, `escrow_salt`, `escrow_nonce` — escrow lives in `user_key_escrow`; `profile_picture_file_key`/`profile_banner_file_key` — replaced by `encrypted_pic_key`), `servers.invite_code` (042's batch aborted before this drop), `server_keys.eph_pub`+`device_id`, `dm_keys.device_id` (multidevice moved to `user_devices`). Code edits: Message/DmMessage structs stripped to live fields; all 10 message-read/save/edit functions reindexed (`list_messages`, `list_messages_before/around`, `list_dm_messages(_before)`, `save_encrypted_message`, `save_dm_message`, `edit_*`, `get_dm_last_message`, admin lists); ws.rs parse/broadcast/save chains updated; REST/WS JSON no longer emits dead fields; `save_server_key`/`save_dm_key` dropped the always-`''` `device_id` param; admin users dump collapsed 23→21 cols. Verified: cargo build clean (only pre-existing `username` warnings), live DB shows all 26 columns gone, server HTTP 200, reviewer approved, automated row.get↔SELECT column-count checker passes for all 52 query closures (remaining flagged pairs are intentional username/profile-pic skips + windowing artifacts).
