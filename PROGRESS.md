...
### 155. Android haptics actually buzz now, and notifications stop lingering

**Why no haptic cue ever worked in the app.** Every cue — incoming call,
ring→waiting, notification, and the Settings → "Test pattern" buttons — was
`navigator.vibrate(...)`. Chromium **disabled the Vibration API on Android in
v79** and left the interface in place: inside the WebView the call is defined, is
not blocked, returns `true` while the page is visible, and does nothing. So the
cues worked in a browser and were silently dead in the app, and the Test buttons
sat exactly where a user would go to check. The permission was never the
problem (there was no `VIBRATE` at all) — but the permission alone would not have
fixed it either.

**The fix** is the `box-shell` plugin (already registered, already granted to the
remote origin) growing a `vibrate` command: `VibrationEffect.createWaveform`
plays the app's existing `[buzz, pause, buzz, …]` arrays as one hardware
waveform, through `VibratorManager` on API 31+ and `Vibrator` below it. `VIBRATE`
ships in the plugin manifest (install-time — no prompt); `allow-vibrate` and
`allow-clearNotifications` joined its `default.toml`. On the page,
`window.boxHaptic` / `window.boxBuzz` (`static/box-shell.js`) are now the single
entry point for all eight call sites, falling back to `navigator.vibrate` in a
browser and on desktop so those behave exactly as before.

**Notifications.** Two behaviours, both Android-only:

* While the app is **in front** it posts no system notification — the badge,
toast and sound already said it, and one posted anyway outlived the visit (the
notification plugin's shim posts a plain object with no `close()`), which is why
read notifications piled up in the shade.
* Coming **back** to the app calls `clearNotifications`, which cancels every
active notification except the ongoing call's (`4711`, owned by
`CallForegroundService`; kept in step by a documented constant in the plugin).
The in-app unread state is deliberately untouched — the notification box and the
badges are cleared by *opening that channel or DM*, which is exactly the state a
user returns to the app to see. `tests/box-device.spec.ts` pins that.

**Verified locally, not by the release.** The Android APK was built on this
machine (~4 min): the merged manifest carries `android.permission.VIBRATE` and
the release dex still contains `vibrate` / `clearNotifications` / `BoxShellPlugin`
after R8 — i.e. the Kotlin compiles and survives minification. The desktop bundle
was built locally the same way. `tests/box-device.spec.ts` adds 10 tests (the
bridge in isolation, plus on the real page: the Test-pattern button buzzes the
phone, nothing is posted while in front and one is posted while hidden, coming
back clears the shade, and the unread inbox survives the clear); the existing
`call-haptics-battery` + `call-indicators-live` suites stay green (10/10).

**Research: "stay alive after I close it"** (§3c of `android-templates/README.md`).
A foreground service is the only way to keep the socket alive with the app
closed — permanent undismissable notification, continuous battery, cannot be
started from the background (Android 12+), a platform-approved type plus Play's
use-case declaration (Android 14+), and OEM battery managers kill it anyway. Push
(FCM) is the sanctioned way to be woken while dead, and needs the one thing this
project cannot create: a Firebase project plus a server-side sender. The
credential-free middle ground is an opt-in `dataSync` service.

**Files:** `src-tauri/plugins/box-shell/android/src/main/java/com/e2echat/boxshell/BoxShellPlugin.kt`,
`src-tauri/plugins/box-shell/android/src/main/AndroidManifest.xml`,
`src-tauri/plugins/box-shell/permissions/default.toml`,
`src-tauri/plugins/box-shell/permissions/autogenerated/commands/{vibrate,clearNotifications}.toml`,
`src-tauri/plugins/box-shell/permissions/autogenerated/reference.md`,
`src-tauri/plugins/box-shell/src/lib.rs`, `static/box-shell.js`, `static/voice.js`,
`static/chat.js`, `static/roles.js`, `static/thread_categories_shortcuts.js`,
