...
| ---- | ----- | ---------------- |
| 13.6 Android screen share | **not fixed.** Two candidate causes, both needing device evidence to tell apart: wry's WebChromeClient may auto-deny the WebView's display-capture request, or the picker opens and the returned track never reaches the senders. The fix differs completely per cause, and guessing risks the audio path that is currently working. | `adb logcat` filtered for `onPermissionRequest` / `MediaProjection` while pressing Share screen |
