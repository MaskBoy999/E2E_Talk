...
| | Item |
|---|---|
| ❌ | **The published v0.2.9 APK is debug-signed: the secrets are not visible to the workflow.** Proven by downloading the release asset — `apksigner` reports `CN=Android Debug`, while the local build of the *same commit* is `CN=E2E Chat Upload Key`; the run's own annotation says *"No ANDROID_KEYSTORE_BASE64 secret — signing with a throwaway debug key"*. So the three values are missing, misnamed, or were added under **Variables** instead of **Secrets** (Settings → Secrets and variables → **Actions** → *Secrets* tab). The workflow now **refuses to publish a tagged release** when the secret is absent (a manual run still produces a debug-signed test artifact), so this can no longer happen silently. **Do not install the published v0.2.9 APK if you intend to update later** — install the locally signed `dist/E2E-Chat-v0.2.9-android.apk` (same upload key as every future release) until a tagged build is signed properly. |
